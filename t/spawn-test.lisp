...
(describe
 ...
 (it
  ...
  (let ((process (spawn "/bin/sh" (list "-c" "sleep 5"))))
    ...
    (expect (process-alive-p process) :to-be nil)))
 (it
  "process-kill stops a running process via SIGKILL"
  (let ((process (spawn "/bin/sh" (list "-c" "trap '' TERM; sleep 5"))))
    ...
    (expect (process-alive-p process) :to-be nil)))
 (it
  "does not misclassify very short processes as launch failures"
  (loop repeat 100
...
(describe
 "process handles and streams"
 (it
  "spawn captures stdout via :output :stream for manual reading"
  (let ((process (spawn "/bin/sh" (list "-c" "printf hello") :output :stream)))
    (process-wait process)
    (expect
     (string= (read-line (process-kit::%process-output process) nil "") "hello")
     :to-be-truthy)))
 (it
  "spawn verifies a private process group before returning"
  (let ((process (spawn "/bin/sh" (list "-c" "sleep 5"))))
    (unwind-protect (let ((pid (process-id process)))
...
(describe
 "communicate result caching"
 (it
  "preserves rich communicate results after process-wait"
 ...
 (%it-process-group-case
  ...
  (let* ((process
    ...
    (unwind-protect (progn
                      (loop repeat 100
                            until (eq
                                   (process-kit::%process-state-communication-status
                      ...
                      (expect
                       (string= (process-result-stdout thread-result) "finished")
                       :to-be-truthy))
      (when (sb-thread:thread-alive-p thread)
        (close-process process)
        (sb-thread:join-thread thread)))))
 (it
  "preserves cached communicate identity and rejects changed contracts"
  (let ((process
         (process-kit::%make-process-handle :raw-process nil :pid 1 :pgid 1 :program "test"))
    ...
    (handler-case (progn
      ...
      (communicate-options-mismatch (condition)
        ...
        (expect
         (getf (communicate-options-mismatch-expected-options condition) :input)
         :to-equal
         "abc")
        (expect
         (getf (communicate-options-mismatch-actual-options condition) :input)
         :to-equal
         "changed"))))))
...
(describe
 "structured logging"
 (it
  "*process-logger* observes a successful spawn and a launch failure"
  (let* ((output (make-string-output-stream))
         (process-kit:*process-logger*
          (log-kit:make-logger
           ...
           "test"
           :handler
           (make-instance 'log-kit:text-handler :stream output)
           :level
           log-kit:+level-info+)))
    (let ((process (spawn "/bin/sh" (list "-c" "exit 0"))))
      (process-wait process))
 ...
 (it "*process-logger* stays silent by default" (expect process-kit:*process-logger* :to-be nil)))
...
(describe
 ...
 #-linux
  (it "kills descendants after the process-group leader exits on TERM"
    (let ((process (spawn "/bin/sh"
                          ...
                          :output :stream :error :stream)))
      (let ((result (communicate process :timeout 0.3d0 :grace-period 0.3d0 :on-timeout :return)))
        (expect result :to-have-timed-out)
        (expect (process-kit::%process-group-alive-p process) :to-be nil))))
 (it
  "close-process kills descendants after their leader exits"
  (let ((process
         (spawn "/bin/sh" (list "-c" "trap \"exit 0\" TERM; (trap \"\" TERM; sleep 5) & wait"))))
    (sleep 0.05)
    (close-process process :timeout 0.3d0)
    (expect (process-kit::%process-group-alive-p process) :to-be nil)))
 (it
  "communicate signals process-timeout-error on its default :on-timeout :error"
  (with-process
   (process (%spawn-sleeping))
 ...
 (%it-process-group-case
  ...
  (%with-process-cleanup
   (process (spawn "/bin/sh" (list "-c" "(trap '' TERM; sleep 5) & exit 0")))
   (process-wait process)
 ...
 (%it-process-group-case
  ...
  (%with-process-cleanup
   (leader-only (spawn "/bin/sh" (list "-c" "(trap '' TERM; sleep 5) & wait")))
   (sleep 0.05)
 ...
 (%it-process-group-case
  ...
  (%with-process-cleanup
   (process (spawn "/bin/sh" (list "-c" "(trap \"\" TERM; sleep 30) & exit 0")))
   (process-wait process)
   (sleep 5.2d0)
   (expect (process-kit::%process-group-alive-p process) :to-be-truthy)
   (close-process process :timeout 0.3d0)
...
(describe
 ...
 (it
  "rejects unsafe spawn strings and environments"
 ...
 (it
  "rejects unsafe command environment updates"
  (signals error (make-command "/bin/true" nil :environment-update (list (cons "A=B" "value"))))
  (signals
   error
   (make-command "/bin/true" nil :environment-update (list (cons "A" "1") (cons "A" "2"))))))

(describe
 "spawn :fd-limit"
 (it
  "rejects a non-positive or non-integer fd-limit"
 ...
 (it
  "lowers the child's inherited RLIMIT_NOFILE without changing this process's own limit"
  (multiple-value-bind (before-soft before-hard) (process-kit::%get-nofile-limit)
    (let ((result (run "sh" (list "-c" "ulimit -n") :search t :fd-limit 1024)))
      (expect
       (string= (string-trim '(#\Newline) (process-result-stdout result)) "1024")
       :to-be-truthy))
 ...
 (it
  "restores this process's own fd limit even when the spawn itself fails"
  (multiple-value-bind (before-soft before-hard) (process-kit::%get-nofile-limit)
    (signals
     process-launch-error
     (spawn
      "definitely-not-a-cl-process-kit-program"
      nil
      :search
      t
      :environment
...
(describe
 ...
 (it
  "resolves searched programs from the effective PATH"
  (dolist (case (list
                 (list '("PATH=/bin") nil)
                 (list '("PATH=") #P"/bin/")
                 (list '("PATH=bin") #P"/")))
 ...
 (it
  "rejects missing and non-executable searched programs"
  (signals
   process-launch-error
   (spawn "definitely-not-a-cl-process-kit-program" nil :search t :environment (list "PATH=/bin")))
  (signals process-launch-error (spawn "hosts" nil :search t :environment (list "PATH=/etc"))))
 (it
